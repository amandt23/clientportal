...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Edit Proofreader Information') }}</h4>
			<ol class="breadcrumb mb-2">
				<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="fa-solid fa-id-badge mr-2 fs-12"></i>{{ __('Admin') }}</a></li>
			
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('admin.proofreaders') }}">{{ __('Proofreader List') }}</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="#"> {{ __('Edit Proofreader') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
...
@section('content')
	...
	<div class="row">
		<div class="col-xl-9 col-lg-8 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Edit Proofreader Information') }}</h3>
				</div>
				<div class="card-body pb-0">
					<form method="POST" action="{{ route('admin.user.update', [$user->id]) }}" enctype="multipart/form-data">
						...
						<div class="card-footer border-0 text-right mb-2 pr-0">							
							<a href="{{ route('admin.proofreaders') }}" class="btn btn-cancel mr-2">{{ __('Return') }}</a>
		...
		<div class="col-xl-3 col-lg-4 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Edit Proofreader Settings') }}</h3>
				</div>
				<div class="card-body">
							...
							<div class="col-sm-12 col-md-12">
								<div class="form-group">
									...
									<select id="user-group" name="group" data-placeholder="{{ __('Select User Group') }}">	
										...
										<option value="proofreader" {{ ($user->group == 'proofreader') ? 'selected' : '' }}>{{ __('Proofreader') }}</option>
