...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Edit Trainer Information') }}</h4>
			<ol class="breadcrumb mb-2">
				<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="fa-solid fa-id-badge mr-2 fs-12"></i>{{ __('Admin') }}</a></li>
			
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('admin.trainers') }}">{{ __('Trainer List') }}</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="#"> {{ __('Edit Trainer') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
...
@section('content')
	...
	<div class="row">
		<div class="col-xl-9 col-lg-8 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Edit Trainer Information') }}</h3>
				</div>
				<div class="card-body pb-0">
					<form method="POST" action="{{ route('admin.trainer_update', [$user->id]) }}" enctype="multipart/form-data">
						@method('PUT')
						@csrf
						<div class="row">
							<div class="col-sm-6 col-md-6">
								<div class="input-box">
									<div class="form-group">
										...
										<input type="text" class="form-control @error('username') is-danger @enderror" name="username" id="username" value="{{$user->username}}" readOnly required>
										</div>
										@error('username')
						...
						<div class="card-footer border-0 text-right mb-2 pr-0">							
							<a href="{{ route('admin.trainers') }}" class="btn btn-cancel mr-2">{{ __('Return') }}</a>
		...
		<div class="col-xl-3 col-lg-4 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Edit Trainer Settings') }}</h3>
				</div>
				<div class="card-body">
					<form method="POST" action="{{ route('admin.user.change', [$user->id]) }}" enctype="multipart/form-data">
							...
							<div class="col-sm-12 col-md-12">
								<div class="form-group">
									...
									<select id="user-group" name="group" data-placeholder="{{ __('Select User Group') }}">	
										...
										<option value="trainer" {{ ($user->group == 'trainer') ? 'selected' : '' }}>{{ __('Trainer') }}</option>
						...
						<div class="card-footer border-0 text-right pb-0 pr-0">							
							<a href="{{ route('admin.user.admin') }}" class="btn btn-cancel mr-2">{{ __('Return') }}</a>
